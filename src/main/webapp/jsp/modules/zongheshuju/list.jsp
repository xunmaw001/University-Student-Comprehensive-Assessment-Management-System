...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">综合数据管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>综合数据列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="btns">
				...
				<button onclick="chartDialog1()" type="button" class="btn btn-info btn-statis" v-if="crossBtnControl('综合成绩统计')"><i class="fa fa-database"></i><span>综合成绩统计</span></button>
			</div>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('chengjizhanbi')">成绩占比<i id="chengjizhanbiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('kaoqinchengji')">考勤成绩<i id="kaoqinchengjiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('kaoqinzhanbi')">考勤占比<i id="kaoqinzhanbiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('ketangchengji')">课堂成绩<i id="ketangchengjiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('ketangzhanbi')">课堂占比<i id="ketangzhanbiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('zonghechengji')">综合成绩<i id="zonghechengjiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('chengjipingyu')">成绩评语<i id="chengjipingyuIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('lururiqi')">录入日期<i id="lururiqiIcon" class="fa fa-sort"></i></th>
						<th>操作</th>
					</tr>
				</thead>
				<tbody></tbody>
            ...
            <div class="openBox" v-if="showType1">
                ...
                <div class="openView">
                    <div id="zonghechengjiMain1" class="graph" style="width: 70vw;height:50vh;"></div>
                </div>
            </div>
		</div>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var chengjizhanbiCell  = document.createElement('td');

						 chengjizhanbiCell.innerHTML = item.chengjizhanbi;

			row.appendChild(chengjizhanbiCell);
			var kaoqinchengjiCell  = document.createElement('td');

						 kaoqinchengjiCell.innerHTML = item.kaoqinchengji;

			row.appendChild(kaoqinchengjiCell);
			var kaoqinzhanbiCell  = document.createElement('td');

						 kaoqinzhanbiCell.innerHTML = item.kaoqinzhanbi;

			row.appendChild(kaoqinzhanbiCell);
			var ketangchengjiCell  = document.createElement('td');

						 ketangchengjiCell.innerHTML = item.ketangchengji;

			row.appendChild(ketangchengjiCell);
			var ketangzhanbiCell  = document.createElement('td');

						 ketangzhanbiCell.innerHTML = item.ketangzhanbi;

			row.appendChild(ketangzhanbiCell);
			var zonghechengjiCell  = document.createElement('td');

						 zonghechengjiCell.innerHTML = item.zonghechengji;

			row.appendChild(zonghechengjiCell);
			var chengjipingyuCell  = document.createElement('td');

						 chengjipingyuCell.innerHTML = item.chengjipingyu;

			row.appendChild(chengjipingyuCell);
			var lururiqiCell  = document.createElement('td');

						  lururiqiCell.innerHTML = item.lururiqi;


			row.appendChild(lururiqiCell);
	...
	// 导出
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("zongheshuju/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"zongheshuju");
		...
		window.location.href = url;
                ...
                function chartDialog1(){
                    ...
                    $.ajax({
                        type: "GET",
                        url: baseUrl+"zongheshuju/sectionStat/zonghechengji/kechengmingcheng",
                            ...
                            if(data.code == 0){
                                if(data.data != null){
                                    ...
                                    for(let i=0;i<res.length;i++){
                                        ...
                                        yAxis3.push(parseFloat((res[i].优)));
                                        pArray.push({
                                            name: res[i].zonghechengji
                                        })
                                    }

                                    var myChart = echarts.init(document.getElementById('zonghechengjiMain1'),'macarons');
                                    var option = {};
                                    option = {
                                        title: {
                                            text: '综合成绩统计',
